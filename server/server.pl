...
:- http_handler(root(api/ping), h_api_ping, [method(get)]).
...
allow_cors(Request) :-
    cors_enable(Request, [methods([get, post, options]), origins(['*'])]).
...
serve_app_js(_Request) :-
    http_reply_file('./static/app.js', [unsafe(true), mime_type('application/javascript')], []).

server(Port) :-
    ignore(stop_all),
    catch(http_server(http_dispatch, [port(Port), encoding(utf8)]),
          E,
